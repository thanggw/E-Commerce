...
document.querySelector(".btn-cart").addEventListener("click", async () => {
    ...
    if (!selectedColor || !selectedSize) {
        Swal.fire("Lỗi", "Vui lòng chọn topping và size!", "warning");
        return;
    }
    ...
    try {
        ...
        const response = await fetch("http://localhost:8082/api/carts/add", {
            method: "POST",
            credentials: "include",
            headers: {
                "Content-Type": "application/json"
            ...
            body: JSON.stringify({
                productId: parseInt(productId),
                colorId: parseInt(selectedColor.dataset.id),
                sizeId: parseInt(selectedSize.dataset.id),
                quantity: quantity
            })
        });
        ...
        await Swal.fire("Thành công", "Đã thêm vào giỏ hàng!", "success");
        getCart(); // Cập nhật số lượng trên icon giỏ hàng
    } catch (error) {
        console.error("Error:", error);
...
async function checkCanReview() {
    const response = await fetch(`/api/products/can-review?userId=${userId}&productId=${productId}`);
    const canReview = await response.json();
    if (canReview) {
        document.getElementById('toggle-review-form').style.display = 'block';
    } else {
        document.getElementById('toggle-review-form').style.display = 'none';
    }
}

checkCanReview(); // Gọi hàm này sau khi lấy được productId và userId
